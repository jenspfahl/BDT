...
// Forked from SlidingControl.dart to add adjustable corner radius
...
///
/// {@tool dartpad}
/// This example shows a [SlidingControl] with an enum type.
///
...
class SlidingControl<T extends Object> extends StatefulWidget {
  ...
  @override
  State<SlidingControl<T>> createState() => _SegmentedControlState<T>();
}

class _SegmentedControlState<T extends Object> extends State<SlidingControl<T>>
    with TickerProviderStateMixin<SlidingControl<T>> {
  ...
  @override
  void didUpdateWidget(SlidingControl<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
    ...
    return UnconstrainedBox(
      ...
      child: Container(
        ...
        decoration: ShapeDecoration(
          shape: RoundedSuperellipseBorder(borderRadius: BorderRadius.all(widget.cornerRadius ?? _kCornerRadius)),
...
class _RenderSegmentedControl<T extends Object> extends RenderBox
  ...
  void _paintThumb(PaintingContext context, Offset offset, Rect thumbRect) {
    ...
    final RSuperellipse thumbShape = RSuperellipse.fromRectAndRadius(
      ...
      Radius.elliptical(_kThumbRadius.x + (cornerRadius?.x??0), _kThumbRadius.y + (cornerRadius?.y??0)),
    );
